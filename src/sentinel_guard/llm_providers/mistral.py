...
class MistralModelProvider:
    ...
    def generate(
        ...
        if tool_choice == "required" and not self.supports_forced_tool_calling:
            raise SentinelGuardError("Mistral provider does not support forced tool calling.")
        ...
        try:
            ...
            raise SentinelGuardError(
                f"Mistral API error: {exc.code} {exc.reason} for model '{self.model}': {body}"
            ) from exc
        except urllib.error.URLError as exc:
            raise SentinelGuardError(f"Mistral API connection error: {exc.reason}") from exc
    ...
    def _send_request(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise SentinelGuardError("Mistral API returned malformed JSON.") from exc
    ...
    def _extract_text(self, data: dict[str, Any]) -> str:
        ...
        raise SentinelGuardError("Unable to extract text output from Mistral API response.")
