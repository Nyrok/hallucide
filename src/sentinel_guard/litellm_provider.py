...
class LiteLLMModelProvider:
    ...
    def generate(
        ...
        if tool_choice == "required" and not self.supports_forced_tool_calling:
            raise SentinelGuardError("LiteLLM provider does not support forced tool calling.")
        ...
        try:
            ...
            raise SentinelGuardError(f"LiteLLM error for model '{self.model}': {exc}") from exc
    ...
    def _extract_text(self, response: Any) -> str:
        ...
        raise SentinelGuardError("Unable to extract text output from LiteLLM response.")
