...
class SentinelGuardError(Exception):
    """Base exception for sentinel guard failures."""


class VerificationError(SentinelGuardError):
...
class RetrievalError(SentinelGuardError):
    """Raised during retrieval state or document handling issues."""


class InvalidClaimError(SentinelGuardError, ValueError):
